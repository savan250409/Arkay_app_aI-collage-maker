                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center border-bottom pb-3 mb-4">
                        <div class="d-flex align-items-center">
                            <i class="mdi mdi-image text-primary" style="font-size: 2rem; margin-right: 15px;"></i>
                            <div>
                                <h3 class="text-primary mb-0" style="font-weight: bold;">Background Management</h3>
                                <small class="text-muted">Manage backgrounds</small>
                            </div>
                        </div>
                        <div class="d-flex align-items-center" style="gap: 1rem;">
                            <div class="bg-light px-3 py-2 rounded mr-3 text-dark font-weight-bold"
                                style="border: 1px solid #e3e3e3;">
                                <i class="mdi mdi-layers-outline mr-1"></i> Total: {{ $backgrounds->total() }} Backgrounds
                            </div>
                            <a href="{{ route('backgrounds.create') }}" class="btn btn-primary btn-sm btn-icon-text"
                                style="padding: 0.5rem 0.8rem;">
                                <i class="mdi mdi-plus btn-icon-prepend"></i> Add Background
                            </a>
                        </div>
                    </div>
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    @foreach($backgrounds as $background)
                                        <tr>
                                            <td>{{ $background->id }}</td>
                                            <td>{{ $background->category->name ?? 'N/A' }}</td>
                                            <td>
                                                @if(!empty($background->images) && is_array($background->images))
                                                    @php
                                                        $imageNames = array_map(fn($item) => is_array($item) ? $item['image'] : $item, $background->images);
                                                    @endphp
                                                    <div class="d-flex align-items-center flex-wrap">
                                                        @foreach(array_slice($imageNames, 0, 2) as $img)
                                                            <div class="mr-2 mb-2">
                                                                <img src="{{ asset('upload/background/' . ($background->category->name ?? 'default') . '/backgrounds/' . $img) }}"
                                                        ...
                                                        @if(count($imageNames) > 2)
                                                            <div class="view-more-btn mb-2"
                                                                ...
                                                                data-category="{{ $background->category->name ?? 'Category' }}"
                                                                data-path="{{ asset('upload/background/' . ($background->category->name ?? 'default') . '/backgrounds/') }}"
                                                                ...
                                                                <span style="font-weight: bold; font-size: 11px; text-transform: uppercase;">VIEW MORE</span>
                                                                <span style="font-weight: bold; font-size: 18px;">(+{{ count($imageNames) - 2 }})</span>
                                                            </div>
                                                        @endif
                                                    </div>
                                                @else
                                            ...
                                            <td>
                                                <a href="{{ route('backgrounds.edit', $background->id) }}"
                                                    class="btn btn-warning btn-sm">Edit</a>
                                                <button class="btn btn-danger btn-sm delete-btn" data-id="{{ $background->id }}"
                                                    data-url="{{ route('backgrounds.destroy', $background->id) }}">Delete</button>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                        ...
                        <div class="row mt-3 align-items-center">
                            <div class="col-sm-6">
                                <div class="dataTables_info" id="order-listing_info" role="status" aria-live="polite">
                                    Showing {{ $backgrounds->firstItem() ?? 0 }} to {{ $backgrounds->lastItem() ?? 0 }} of
                                    {{ $backgrounds->total() }} entries
                                </div>
                            </div>
                            <div class="col-sm-6 d-flex justify-content-end">
                                <div class="dataTables_paginate paging_simple_numbers" id="order-listing_paginate">
                                    {!! $backgrounds->appends(request()->query())->links('pagination::bootstrap-4') !!}
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="modal-dialog modal-lg" role="document" style="max-width: 90%;">
            <div class="modal-content">
                <div class="modal-header border-bottom-0">
                    <h5 class="modal-title font-weight-bold" id="galleryModalLabel">Background Images</h5>
            ...
            $(document).on('click', '.delete-btn', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        Swal.fire({
                            title: 'Final Warning!',
                            text: "This will permanently delete this background and its image files. Are you really sure?",
                            icon: 'error',
                            showCancelButton: true,
                            ...
                            if (secondResult.isConfirmed) {
                                $.ajax({
                                    ...
                                    success: function (response) {
                                        if (response.success) {
                                            Swal.fire('Deleted!', 'Background and its files have been deleted.', 'success');
                                            row.remove();
                                        }
                                    }
                                });
