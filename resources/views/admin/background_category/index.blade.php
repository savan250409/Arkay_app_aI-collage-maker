                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center border-bottom pb-3 mb-4">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                <h3 class="text-primary mb-0" style="font-weight: bold;">Background Category Management</h3>
                                <small class="text-muted">Manage background categories</small>
                            </div>
                        </div>
                        <div class="d-flex align-items-center" style="gap: 1rem;">
                            <a href="{{ route('background-categories.order') }}" class="btn btn-info btn-sm btn-icon-text"
                            ...
                            <a href="{{ route('background-categories.create') }}" class="btn btn-primary btn-sm btn-icon-text"
                                style="padding: 0.5rem 0.8rem;">
                                ...
                                <tbody>
                                    @foreach($categories as $category)
                                        <tr>
                                            ...
                                            <td>{{ $category->name }}</td>
                                            <td>
                                                <img src="{{ asset('upload/background/' . $category->name . '/category-thumbnail-image/' . $category->image) }}"
                                            ...
                                            <td>
                                                <a href="{{ route('background-categories.edit', $category->id) }}"
                                                    class="btn btn-warning btn-sm">Edit</a>
                                                <button class="btn btn-danger btn-sm delete-btn" data-id="{{ $category->id }}"
                                                    data-url="{{ route('background-categories.destroy', $category->id) }}">Delete</button>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '.delete-btn', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        Swal.fire({
                            title: 'Final Warning!',
                            text: "This will permanently delete ALL backgrounds and files associated with this category. Are you really sure?",
                            icon: 'error',
                            showCancelButton: true,
                                ...
                                $.ajax({
                                    ...
                                    success: function (response) {
                                        if (response.success) {
                                            Swal.fire(
                                                'Deleted!',
                                                'Category and all associated backgrounds have been deleted.',
                                                'success'
                                            );
                                            row.remove();
                                        }
            ...
            $(document).on('change', '.status-toggle', function () {
                ...
                $.ajax({
                    url: "{{ route('background-categories.update-status') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: id,
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            Toast.fire({
                                icon: 'success',
                                title: status ? 'Category set to Pro (all backgrounds updated to Pro)' : 'Category set to Free'
                            });
                        }
                    }
                });
            ...
            $(document).on('change', '.active-status-toggle', function () {
                ...
                $.ajax({
                    url: "{{ route('background-categories.update-active-status') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: id,
            ...
            function fetch_data(page) {
                ...
                $.ajax({
                    url: "{{ route('background-categories.index') }}",
                    data: {
                        page: page,
                        search: search,
                        per_page: per_page
